"""Nuitka build script for TE Lab Equipment Inventory Manager.

Usage:
    python build.py                   # standard build
    python build.py --recreate-venv   # wipe and recreate the build venv first
    python build.py --sign            # also code-sign the output exe
    python build.py --version 1.2.0   # set a custom version string
"""
...
def install_deps(python_exe: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "pip", "setuptools", "wheel", "nuitka", "ordered-set", "zstandard",
